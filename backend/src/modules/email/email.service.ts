...
export class EmailService {
  ...
  constructor() {
    ...
    if (smtpUser && smtpPass) {
      ...
      this.logger.log(`Initialized Production SMTP Mailer [${smtpHost}:${smtpPort}] (${smtpUser})`);
    } else {
      // Fallback transport khi chưa cấu hình env
      this.transporter = nodemailer.createTransport({
        jsonTransport: true,
      });
      this.logger.warn('SMTP credentials not configured. Running in Dev Console mode.');
    }
  }
  ...
  async sendOtpEmail(toEmail: string, otp: string): Promise<boolean> {
    ...
    try {
      ...
      this.logger.log(`[SMTP] OTP Email successfully sent to ${toEmail} | MessageId: ${info.messageId}`);
      return true;
    } catch (err) {
